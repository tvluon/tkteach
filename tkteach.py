...
class tkteach:
    ...
    def __init__(self, master):
        ...
        self.imgStage = tk.Label(
            self.frameMIDDLE, text="", height=self.default_size[1], width=self.default_size[0])
        self.imgStage.pack()
        ...
        self.prevImageButton = tk.Button(
            self.frameMIDDLEBUTTONS, text="<- Save & Previous", command=self.prevImage, state=tk.DISABLED)
        self.prevImageButton.pack(side=tk.LEFT)

        self.nextImageButton = tk.Button(
            self.frameMIDDLEBUTTONS, text="Save & Next ->", command=self.nextImage, state=tk.DISABLED)
        ...
        self.imageNumberLabel = tk.Label(
            self.frameMIDDLEIMGLABEL, text="Image Number:")
    ...
    # def initializeDatasets(self):
        ...
        # Get Dataset
        # datasetDir = './dataset'
        # self.dataSetsListDir = [os.path.join(datasetDir, o) for o in os.listdir(datasetDir) if os.path.isdir(os.path.join(datasetDir,o))]
    ...
    def initializeCategories(self):
        ...
        try:
            catFile = open(self.catFilename, 'r')
            self.categories = [o.strip().split()[0]
                               for o in catFile.readlines()]
            catFile.close()
        except IOError:
            self.categories = []
    ...
    def loadImage(self):
        ...
        self.imgFileName.config(
            text=str(self.imageListDir[self.imageSelection]))
    ...
    def saveImageCategorization(self):
        ...
        cati = self.categoriesListbox.curselection()[0]
    ...
    def loadDataSet(self):
        ...
        if not metadf is None:
            self.imageListDir = metadf['image_path'].tolist()
            ...
            # Communicate:
            self.statusBar.config(text='Number of images: '+str(len(metadf)))
            print('---->Dataset loaded. Number of images: '+str(len(metadf)))
